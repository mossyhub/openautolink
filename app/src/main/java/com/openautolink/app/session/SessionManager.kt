        ...
        decoder.needsKeyframe.collect { needed ->
            if (needed) {
                ...
                while (decoder.needsKeyframe.value) {
                    ...
                    // it (post phone-call / GM-UI takeover, or a cold-connect stall) —
                    // so it emits no IDR and video stays frozen until the next natural
                    // GOP (~2 min). Use the focus-bounce variant (NATIVE_TRANSIENT ->
                    // PROJECTED, a real transition) on the first attempt and
                    // periodically thereafter to actually pull a fresh IDR forward;
                    // plain re-requests in between.
