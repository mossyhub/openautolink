...
OpenAutoLink is an Android Automotive OS (AAOS) app that wirelessly projects Android Auto or CarPlay from your phone onto your car's infotainment screen via a bridge device on your local network. This privacy policy explains what data the app accesses, how it is used, and how it is protected.
...
The app captures audio from the car's built-in microphone to enable hands-free voice commands (e.g., "Hey Google", "Hey Siri") and phone calls through Android Auto or CarPlay. Microphone audio is sent only to the bridge device on your local network, which forwards it to your phone's session. **Audio is never recorded, stored, or transmitted outside your local network.**
...
The app reads vehicle sensor data (speed, gear, battery level, temperature, etc.) from the car's Vehicle HAL to forward to your phone's Android Auto or CarPlay session. This enables the phone to display accurate vehicle information. **Vehicle data is sent only to the bridge device on your local network and is never stored or transmitted externally.**
...
The app reads accelerometer, gyroscope, and magnetic field sensors from the car's head unit hardware to provide inertial measurement data to Android Auto or CarPlay. This improves navigation accuracy during GPS signal loss (tunnels, parking garages). This sensor data is sent only to the bridge device on your local network.
